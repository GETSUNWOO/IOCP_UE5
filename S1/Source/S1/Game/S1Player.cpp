...
void AS1Player::Tick(float DeltaSeconds)
{
	...
	if (IsMyPlayer() == false)
	{
		...
		{
			SetActorRotation(FRotator(0, DestInfo->yaw(), 0)); // 이것도 순간적으로 휙 돌게 되는데 보간해주면 좋다.
			AddMovementInput(GetActorForwardVector());
		}
		else
		{
			//TODO
			//목적지까지 가지 못했는데 STATE가 바뀌어서 움직임이 멈춘다면 ?
			// 1. 순간이동 시켜준다. 2. 속도를 엄청빠르게해서 목적지까지 움직여준다.
		}
	}
}
...
{
	if (PlayerInfo->object_id() != 0) //혹시나 object가 세팅된 상태에서
	{
		assert(PlayerInfo->object_id() == Info.object_id()); // objectid가 다르다면 크래시
	}
